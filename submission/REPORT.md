...
| C — QA & Chief Investigator | Dashboard 6 panel, load test, practice/challenge, Metrics → Traces → Logs, evidence và report | Commit `feat: complete member C dashboard and incident evidence` trong nhánh nộp | P95 phản ánh tail latency tốt hơn average; chỉ kết luận root cause khi metric, span và log cùng khớp. |
